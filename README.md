...
## 🚀 Key Features
  ...
  * **🔍 Pure Zero-Shot Inference:** The system does not rely on a static database during inference. It can detect *any* object that can be described in English.
  * **✍️ Manual Entry Mode:** Operators can manually type a description (e.g., "Red Nike Shoes") and the system will instantly generate a detector for it.
  * **🛑 Quantity Mismatch Flags:** The system compares `Detected Count` vs. `Ordered Quantity`. If a mismatch is found (due to occlusion), it flags the order as **"Incomplete / Partial Match"** for human review.
  * **🧹 Anti-Hallucination Filters:** Implements a strict "Background Class" (filtering empty shelves/tape) and Confidence Thresholding (`0.20`) to prevent false positives.
...
5.  **Review:** Check the status badges (✅ MATCH / ❌ MISSING).
...
SMARTBIN/
├── src/
│   ├── inference.py       # Core Zero-Shot Logic (YOLO + CLIP)
│   ├── validate_bin.py    # Evaluation Script for Metrics
│   └── run_subset.py      # Artifact Generator
├── streamlit_app/
│   └── app.py             # User Interface Code
├── processed/
│   ├── asin_text.json     # ASIN to Description Mapping
│   └── master_metadata.csv # Ground Truth Data
├── outputs/
│   └── subset_results.json # Final Submission Artifact
